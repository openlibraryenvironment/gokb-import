...
class KbartProcessor extends AbstractProcessor {
  ...
  private int initData(HashMap options) throws Exception {
    ...
    Paths.get(inputFile).withReader { reader ->
      ...
      for (csvRecord in csv.iterator()) {
        if (csvRecord.size() != csv.getHeaderMap().size()) {
          ...
          for (key in MultipleProcessingThread.KEY_ORDER) {
            if ((key != KbartConnector.KBART_HEADER_ZDB_ID && key != KbartConnector.KBART_HEADER_DOI_IDENTIFIER) || csvRecord.isMapped(key)) {
              identifier = csvRecord.get(key)?.toString()?.trim()
              if (identifier) {
                ...
                bridge.connector.kbartKeys.each { kbk ->
                  kbfs << ["${kbk}": csvRecord.get(kbk).toString()]
                }
                bridge.connector.optionalKbartKeys.each { kbk ->
                  if (csvRecord.isMapped(kbk)) {
                    kbfs << ["${kbk}": csvRecord.get(kbk).toString()]
                  }
                }
                if (kbfs.size() > 0) {
          ...
          if (!identifier) {
            ...
            log.info('Entry "' + "${csvRecord.get('publication_title')}" + '" ignored due to missing identifier.')
            stash.get(Stash.IGNORED_KBART_ENTRIES).add(csvRecord.get('publication_title').toString())
          }
        }
      }
  ...
  private CSVParser getCSVParserFromReader(Reader reader) {
    // Filter BOM before parsing
    String text = reader.getText();
    text = text.replace('\uFEFF', '')
    return CSVParser.parse(text, csvFormat)
  }
